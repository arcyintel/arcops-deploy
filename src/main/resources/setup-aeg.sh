...
echo -e "${YELLOW}set AEG_BASE_URL=https://$AEG_DOMAIN (the prod compose already defaults to the${NC}"
echo -e "${YELLOW}central domain). emm then SELF-BOOTSTRAPS its connector token + webhook secret${NC}"
echo -e "${YELLOW}from its licence (POST /api/v1/aeg/bootstrap) — no manual token registration.${NC}"
echo ""
